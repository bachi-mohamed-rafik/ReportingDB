...
function TableComponentReservation({ data }) {
  const [so, setSo] = useState(null);
  const [count, setCount] = useState(0);
  const [id, setID] = useState(10268);
  ...
  // example
  // https://erp.bessapromotion.com/web?debug=1#id=10268&cids=1&menu_id=281&action=428&model=crm.reservation&view_type=form
  // Base URL
  var baseURL = 'https://erp.bessapromotion.com/web?debug=1';

  // Construct the dynamic URL with the ID variable
  var dynamicURL = baseURL + '#id=' + id + '&cids=1&menu_id=281&action=428&model=crm.reservation&view_type=form';
  // {dynamicURL}
...
export default TableComponentReservation; 
